...
class yonetim {
    ...
    public function urunguncelle($vt) {
        ...
        if ($buton) :
            ...
            @$resim = $_FILES["resim"]["name"]; // Yüklenen resmin adı
            ...
            if ($urunad == "" || $urunid == "" || $fiyat == "" || $katid == "") :
                ...
                if ($resim != "") {
                    ...
                    $hedef_yol = "D:/xampp/htdocs/resimler/" . $resim; 
                    // Resmi sunucuya yükle
                    move_uploaded_file($_FILES["resim"]["tmp_name"], $hedef_yol); 
                
                    // Resim ile birlikte ürünü güncelle
                    // Veritabanında 'resimler/dosya_adı' formatında kaydediyoruz
                    $this->genelsorgu($vt, "UPDATE urunler SET ad = '$urunad', fiyat = $fiyat, katid = $katid, resim = 'resimler/$resim' WHERE id = $urunid");
                } else {
    ...
    ?>
            ...
            echo '<div class="col-md-12 table-light">Yeni Resim Yükle<input type="file" name="resim" class="form-control mt-3"/></div>';
    
            echo '<div class="col-md-12 table-light"><input name="buton" value="Güncelle" type="submit" class="btn btn-success mt-3 mb-3"/></div>
...
public function urunekle($vt) {
    ...
    if ($buton) :
        ...
        @$fiyat = htmlspecialchars($_POST["fiyat"]);
        @$katid = htmlspecialchars($_POST["katid"]);
        @$resim = $_FILES["resim"]; // Resim dosyasını al

        if ($urunad == "" || $fiyat == "" || $katid == "" || $resim["name"] == "") :
            $this->uyari("danger", "Bilgiler boş olamaz", "control.php?islem=urunyon");
        else:
            // Resmi yükle
            $hedef_klasor = 'D:/xampp/htdocs/resimler/';
            $hedef_dosya = $hedef_klasor . basename($resim["name"]);

            // Resim yükleme işlemi
            if (move_uploaded_file($resim["tmp_name"], $hedef_dosya)) {
                // DB kaydı (resim yolunu ekliyoruz)
                $resim_yolu = 'resimler/' . basename($resim["name"]); // Resim yolunu belirtiyoruz
                $this->genelsorgu($vt, "insert into urunler (katid, ad, fiyat, resim) values ($katid, '$urunad', $fiyat, '$resim_yolu')");
                $this->uyari("success", "Ürün Eklendi", "control.php?islem=urunyon");
            } else {
                $this->uyari("danger", "Resim Yüklenemedi", "control.php?islem=urunyon");
            }
        ...
        <?php
            echo '<div class="col-md-12 table-light border-bottom"><h4>ÜRÜN EKLE</h4></div>
                ...
                echo '</select>';
                echo '</div>
                    <div class="col-md-12 table-light">Ürün Resmi <input type="file" name="resim" class="form-control mt-3"/></div>
                    <div class="col-md-12 table-light"><input name="buton" value="EKLE" type="submit" class="btn btn-success mt-3 mb-3"/></div>
                </form>';
        ...
        public function sifredegistir($vt)
        {
            ...
            if ($buton) :
                ...
                if ($eskisifre == "" || $yeni1 == "" || $yeni2 == "") :
                    ...
                    // Şifreleme olmadan eski şifreyi kontrol ediyoruz
                    if ($this->genelsorgu($vt, "SELECT * FROM yonetim WHERE sifre = '$eskisifre'")->num_rows == 0) :
                    ...
                    elseif ($yeni1 != $yeni2) :
                        ...
                        // Şifreyi şifreleme olmadan kaydediyoruz
                        $id = $this->coz($_COOKIE["id"]);
        
                        $this->genelsorgu($vt, "UPDATE yonetim SET sifre = '$yeni1' WHERE id = $id");
                ...
                <div class="col-md-3 text-center mx-auto mt-5 table-bordered">
                    <form action="<?php $_SERVER['PHP_SELF'] ?>" method="post">
                        ...
                        <div class="col-md-12 table-light"><input type="text" name="eskisifre" class="form-control mt-3" required placeholder="Eski Şifreniz"/></div>
                        <div class="col-md-12 table-light"><input type="text" name="yeni1" class="form-control mt-3" required placeholder="Yeni Şifreniz"/></div>
                        <div class="col-md-12 table-light"><input type="text" name="yeni2" class="form-control mt-3" required placeholder="Yeni Şifreniz Tekrar"/></div>
    ...
    public function giriskont($r, $k, $s){
        ...
        if ($sonbilgi->num_rows == 0) :
            ...
            $sorgu = "update yonetim set aktif=1 where kulad = '$k' and sifre = '$s'";
            $sor=$r->prepare($sorgu);
            $sor->execute();
    ...
    // Cookie kontrol kısmı değişmeden kalıyor
